...
# Test TCP con timeout explícito de 3s
# CONNECTED = TCP handshake ok (nftables permitió)
# BLOCKED    = refused o timeout (nftables bloqueó)
...
test_tcp() {
    ...
    # Fallback: si bash no tiene /dev/tcp usar nc si existe
    if [ -z "$result" ]; then
        result=$(docker exec "$container" \
            sh -c "nc -z -w3 $ip $port 2>/dev/null \
                   && echo CONNECTED || echo BLOCKED" 2>/dev/null)
    fi
...
# =============================================================================
header "PRE-CHECK: Verificar conectividad real antes de los tests"
# =============================================================================
echo -e "${YELLOW}► IPs de pep_gateway:${NC}"
docker inspect pep_gateway 2>/dev/null | grep '"IPAddress"' | grep -v '""'

echo -e "${YELLOW}► Envoy escuchando en :8443:${NC}"
docker exec pep_gateway sh -c \
    "ss -tlnp 2>/dev/null | grep 8443 || netstat -tlnp 2>/dev/null | grep 8443 || echo 'no disponible'" 2>/dev/null

echo -e "${YELLOW}► Test directo client_intruso → Envoy:${NC}"
docker exec client_intruso \
    sh -c "timeout 3 bash -c 'echo > /dev/tcp/$ENVOY_IP/8443' 2>&1 \
           && echo 'TCP: CONECTÓ' || echo 'TCP: BLOQUEADO/TIMEOUT'"
...
docker logs firewall_perimeter --tail 15 2>&1
...
[ "$SPLUNK_STATUS" = "200" ] \
    ...
    || echo -e "${YELLOW}Splunk HEC: $SPLUNK_STATUS${NC}"
